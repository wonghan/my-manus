...
export class RunCoordinator {
  ...
  async startRunById(runId: string) {
  ...
  async resumeRunById(runId: string) {
    const run = await this.store.getRun(runId);
    if (!run) {
      throw new Error("Run not found.");
    }
  ...
  async startRun(run: RunRecord) {
    try {
      ...
      if (clarification) {
        await this.store.updateRunStatus(run.id, "waiting_clarification");
        await this.store.setPendingClarification(run.id, {
          runId: run.id,
          prompt: run.prompt,
  ...
  async prepareRunAfterClarification(runId: string, clarification: string) {
    const run = await this.store.getRun(runId);
    const pending = await this.store.getPendingClarification(runId);

    if (!run || !pending) {
      throw new Error("No pending clarification found for this run.");
    }

    await this.store.clearPendingClarification(runId);
    await this.store.setRunPrompt(
      runId,
  ...
  async continueRunAfterClarification(runId: string, clarification: string) {
    ...
    try {
      await this.prepareRunAfterClarification(runId, clarification);
      const refreshedRun = await this.store.getRun(runId);
      if (!refreshedRun) {
        throw new Error("Run not found after clarification update.");
      }
      await this.performResearch(refreshedRun, refreshedRun.prompt);
    } catch (error) {
      await this.failRun(run, error);
    }
  }
  ...
  async requestExportApproval(runId: string) {
    ...
    await this.store.updateRunStatus(run.id, "waiting_approval");
  ...
  async resolveApproval(approvalId: string, decision: "approved" | "rejected") {
    ...
    await this.store.updateApprovalStatus(approvalId, decision);
    await this.store.updateRunStatus(run.id, "completed");
  ...
  private async performResearch(run: RunRecord, effectivePrompt: string) {
    ...
    for (const step of leafSteps) {
      ...
      if (execution.artifact) {
        ...
        await this.store.addArtifact(artifact);
  ...
  private async updateStep(
    ...
    await this.store.setRunSteps(run.id, steps);
  ...
  private async pushAssistantDelta(run: RunRecord, delta: string) {
    await this.store.appendAssistantContent(run.assistantMessageId, delta);
    await this.emit(run.id, {
    ...
    const detail = await this.store.getSessionDetail(run.sessionId);
    const content = detail?.messages.find(
  ...
  private async emit(runId: string, event: AgUiEvent) {
    await this.store.pushRunEvent(runId, event);
    await this.eventBus.publish(runId, event);
  }
  ...
  private async failRun(run: RunRecord, error: unknown) {
    ...
    await this.store.updateRunStatus(run.id, "failed");
